...
`)
	cyoaMap := make(map[string]Chapter)
	Unmarshaller(text, cyoaMap)
	// fmt.Println(introMap["intro"])
  ...
  <div>{{ .Story }}</div>
  {{ range .Options}}
  <div> <a href="/{{ .Chapter }}">Go to {{.Chapter}}</a> - {{ .Text }}</div>
  {{ end }}
  </html>
  `

	tmpl, err := template.New("cyoa").Parse(webtmpl)
	if err != nil {
		log.Print(err)
	}

	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		err = tmpl.Execute(w, cyoaMap["intro"])
	...
	http.HandleFunc("/new-york", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["new-york"])
		if err != nil {
			log.Print(err)
		}
	})
	...
	http.HandleFunc("/denver", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["denver"])
		if err != nil {
			log.Print(err)
		}
	})
	...
	http.HandleFunc("/debate", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["debate"])
		if err != nil {
			log.Print(err)
		}
	})
	...
	http.HandleFunc("/mark-bates", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["mark-bates"])
		if err != nil {
			log.Print(err)
		}
	})
	...
	http.HandleFunc("/sean-kelly", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["sean-kelly"])
		if err != nil {
			log.Print(err)
		}
	})
	...
	http.HandleFunc("/home", func(w http.ResponseWriter, r *http.Request) {
		err := tmpl.Execute(w, cyoaMap["home"])
		if err != nil {
			log.Print(err)
		}
	})
